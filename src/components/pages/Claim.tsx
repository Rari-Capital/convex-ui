...
import {
  flywheels,
  useFlywheelsTotalUSD,
  useMaxUnclaimedByMarkets,
} from "hooks/rewards/useUnclaimedByMarkets";
...
const Claim = () => {
  ...
  const cTokensWithPlugin = useCTokensWithRewardsPlugin(
    marketsDynamicData?.assets?.map(({ cToken }) => cToken)
  );
  const { flywheelRewardsTotals, estimatedGas, call } =
    useMaxUnclaimedByMarkets(cTokensWithPlugin) ?? {};
  ...
  return (
    <Box>
      <Text variant="secondary" display="flex" alignItems="center" my={4}>
        <InfoIcon mr={2} />
